...
import { useRef, useEffect, useMemo, useState, useSyncExternalStore, type CSSProperties } from "react";
...
/** True only on the client, after hydration — avoids the SSR/client mismatch
 * that a `useState(false) + useEffect(() => setState(true))` pattern would need. */
...
export default function Rays({
  ...
  // Lazy initializer, not useMemo: this is a one-time random seed, not a
  // derived value — computing it during render would violate purity rules.
  ...
  const [randomColor1RGB, randomColor2RGB] = randomColors;
  ...
  useEffect(() => {
    ...
    // Scene init runs once on mount; the effect below syncs prop changes
    // into the existing uniforms instead of tearing the scene down.
